    ...
    bot.commands.forEach(async command => {
        ...
        if (command.options?.length >= 1) {
            for (let i = 0; i < command.options.length; i++) {
                ...
                else if(command.options[i].type === "integer") slashcommand[`add${command.options[i].type.slice(0, 1).toUpperCase() + command.options[i].type.slice(1, command.options[i].type.length)}Option`](option => option.setName(command.options[i].name).setDescription(command.options[i].description).setAutocomplete(command.options[i].autocomplete).setRequired(command.options[i].required))
