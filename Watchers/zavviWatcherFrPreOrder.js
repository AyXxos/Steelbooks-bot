...
const getZavviPreorder = async () => {
  const url = "https://fr.zavvi.com/blu-ray-and-dvd/coming-soon.list";
  ...
  try {
    ...
    $('.productListProducts_product').each((index, element) => {
      ...
      const status ="**PRE-COMMANDE MAINTENANT CHEZ ZAVVI FR**"
      const now = new Date();
...
module.exports = async (bot) => {
  ...
  const newSteelbooks = await getZavviPreorder();

  if (newSteelbooks.length === 0) {
    logChannel.send("📭 Aucun nouveau steelbook trouvé sur zavviFrPreOrder.");
    return;
  }
  ...
  logChannel.send(`📦 ${newSteelbooks.length} steelbook(s) envoyé(s) individuellement. De chez zavvi france preorder.`);
};
